...
signal item_dropped(item: ItemStack, offset: Vector2)
signal selection_changed
signal inventory_item_activated(item: ItemStack)
signal inventory_item_clicked(item: ItemStack)
...
func _on_item_property_changed(_item: ItemStack, property: String) -> void:
	var relevant_properties := [
		_KEY_SIZE,
		_KEY_ROTATED,
		_KEY_POSITIVE_ROTATION,
		_KEY_STACK_SIZE,
		_KEY_IMAGE,
		"texture",
		"item_type",
...
func _on_item_dropped(item: ItemStack, drop_position: Vector2) -> void:
	if item == null:
		return

...
signal inventory_item_clicked(item: ItemStack, at_position: Vector2, mouse_button_index: int) ## Emitted when an inventory item has been clicked.
...
func _on_item_property_changed(item: ItemStack, property: String) -> void:
	if property == _KEY_NAME || property == _KEY_STACK_SIZE || property == "item_type":
		set_item_text(inventory.get_item_index(item), _get_item_title(item))
	if property == _KEY_IMAGE || property == "texture" || property == "item_type":
...
func _get_inventory_item(index: int) -> ItemStack:
	assert(index >= 0)
...
func _get_item_title(item: ItemStack) -> String:
	if item == null:
		return ""

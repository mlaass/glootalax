...
func _refresh() -> void:
	...
	_inventory_control.inventory_item_activated.connect(func(item: ItemStack):
		inventory_item_activated.emit(item)
	)
	_inventory_control.inventory_item_clicked.connect(func(item: ItemStack, at_position: Vector2, mouse_button_index: int):
		inventory_item_clicked.emit(item, at_position, mouse_button_index)
	)
	_inventory_control.inventory_item_selected.connect(func(item: ItemStack):

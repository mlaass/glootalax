...
func _on_item_dropped(item: ItemStack, drop_position: Vector2) -> void:
  ...
  if item_slot.get_item() == null:
    item_slot.equip(item)  # equip() handles validation and partial transfers


func _join_stacks(item_dst: ItemStack, item_src: ItemStack) -> bool:
  if item_dst == null:
    return false
...
func _swap_items(item1: ItemStack, item2: ItemStack) -> bool:
  if item_slot.get_item() == null:
    return false

  return ItemStack.swap(item1, item2)

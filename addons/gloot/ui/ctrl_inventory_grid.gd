...
signal item_dropped(item: ItemStack, offset: Vector2) ## Emitted when an item has been dropped onto the 2D grid.
signal selection_changed ## Emitted when the item selection has changed.
signal inventory_item_activated(item: ItemStack) ## Emitted when an inventory item has been double-clicked.
signal inventory_item_clicked(item: ItemStack) ## Emitted when an inventory item has been right-clicked.
signal inventory_item_selected(item: ItemStack) ## Emitted when an inventory item has been selected.
signal item_mouse_entered(item: ItemStack) ## Emitted when the mouse cursor has entered the visible area of an item.
signal item_mouse_exited(item: ItemStack) ## Emitted when the mouse cursor has exited the visible area of an item.
...
signal unsocket_requested(parent_item: ItemStack, slot_id: String, current_gem: ItemStack) ## Emitted when unsocketing is requested (right-click on filled socket).
signal item_socketed(parent_item: ItemStack, slot_id: String, gem_item: ItemStack) ## Emitted when an item has been socketed.
signal item_unsocketed(parent_item: ItemStack, slot_id: String, gem_item: ItemStack) ## Emitted when an item has been unsocketed.
...
## Custom control scene representing an `ItemStack` (must inherit `CtrlInventoryItemBase`). If set to `null`,
...
## Callback called before socketing an item. If set and returns false, socketing is cancelled.
## Signature: func(parent_item: ItemStack, slot_id: String, gem_item: ItemStack) -> bool
var socket_item_callback: Callable = Callable()

## Callback called before unsocketing an item. If set and returns false, unsocketing is cancelled.
## Signature: func(parent_item: ItemStack, slot_id: String, current_gem: ItemStack) -> bool
var unsocket_item_callback: Callable = Callable()
...
func _ready() -> void:
  ...
  _ctrl_inventory_grid_basic.inventory_item_clicked.connect(func(item: ItemStack, at_position: Vector2, mouse_button_index: int):
    inventory_item_clicked.emit(item, at_position, mouse_button_index)
  )
  _ctrl_inventory_grid_basic.inventory_item_selected.connect(func(item: ItemStack):
  ...
  _ctrl_inventory_grid_basic.socket_drop_requested.connect(_on_socket_drop_requested)
  _ctrl_inventory_grid_basic.socket_clicked.connect(_on_socket_clicked)
  _ctrl_inventory_grid_basic.select_mode = select_mode
...
func _get_global_grabbed_item() -> ItemStack:
  ...
  if !is_instance_valid(drag_data):
    return null
  if !(drag_data is ItemStack):
    return null
  return drag_data as ItemStack
...
func _on_socket_drop_requested(parent_item: ItemStack, slot_id: String, gem_item: ItemStack) -> void:
  socket_drop_requested.emit(parent_item, slot_id, gem_item)

  # If callback is set, use it for confirmation
  if socket_item_callback.is_valid():
    if not socket_item_callback.call(parent_item, slot_id, gem_item):
      return

  # Perform the socketing
  if parent_item.socket_item(slot_id, gem_item):
    item_socketed.emit(parent_item, slot_id, gem_item)


func _on_socket_clicked(parent_item: ItemStack, slot_id: String, mouse_button: int) -> void:
  # Right-click to unsocket
  if mouse_button == MOUSE_BUTTON_RIGHT:
    var current_gem := parent_item.get_socketed_item(slot_id)
    if current_gem == null:
      return

    unsocket_requested.emit(parent_item, slot_id, current_gem)

    # If callback is set, use it for confirmation
    if unsocket_item_callback.is_valid():
      if not unsocket_item_callback.call(parent_item, slot_id, current_gem):
        return

    # Perform the unsocketing
    var removed := parent_item.unsocket_item(slot_id)
    if removed != null:
      item_unsocketed.emit(parent_item, slot_id, removed)

...
func _on_item_property_changed(item: ItemStack, property: String) -> void:
	if property == _KEY_WEIGHT || property == _KEY_STACK_SIZE:
		_calculate_occupied_space()
...
static func _can_swap(item_dst: ItemStack, item_src: ItemStack) -> bool:
...
static func _get_item_unit_weight(item: ItemStack) -> float:
	# First check item property override, then item_type.weight, then default
	if item.is_property_overridden(_KEY_WEIGHT):
		return item.get_property(_KEY_WEIGHT, 1.0)
	if item.item_type != null:
		return item.item_type.weight
	return 1.0


## Returns the weight of the given item (i.e. the `weight` property).
static func get_item_weight(item: ItemStack) -> float:
	if item == null:
		return -1.0
...
static func set_item_weight(item: ItemStack, weight: float) -> void:

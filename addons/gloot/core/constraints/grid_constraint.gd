...
func get_item_position(item: ItemStack) -> Vector2i:
...
func set_item_position(item: ItemStack, new_position: Vector2i) -> bool:
...
## set_item_position).
func set_item_position_unsafe(item: ItemStack, new_position: Vector2i) -> void:
...
func get_item_size(item: ItemStack) -> Vector2i:
  var result: Vector2i
  # Check override first, then item_type.size, then default
  if item.is_property_overridden(_KEY_SIZE):
    result = item.get_property(_KEY_SIZE, Vector2i.ONE)
  elif item.item_type != null:
    result = item.item_type.size
  else:
    result = Vector2i.ONE
...
func set_item_rotation(item: ItemStack, rotated: bool) -> bool:
...
func rotate_item(item: ItemStack) -> bool:
...
static func set_item_rotation_direction(item: ItemStack, positive: bool) -> void:
  if positive:
...
func can_rotate_item(item: ItemStack) -> bool:
...
func get_item_rect(item: ItemStack) -> Rect2i:
...
func set_item_rect(item: ItemStack, new_rect: Rect2i) -> bool:
...
## Returns the size of an item from an ItemType resource.
static func get_item_type_size(item_type: ItemType) -> Vector2i:
  if item_type == null:
    return Vector2i.ONE
...
func add_item_at(item: ItemStack, position: Vector2i) -> bool:
...
## Creates and adds an item to the inventory at the given position.
func create_and_add_item_at(item_type: ItemType, position: Vector2i) -> ItemStack:
  assert(inventory != null, "Inventory not set!")
  var item_rect := Rect2i(position, get_item_type_size(item_type))
  if !rect_free(item_rect):
    return null

  var item = inventory.create_and_add_item(item_type)
  if item == null:
    return null
...
func get_item_at(position: Vector2i) -> ItemStack:
  assert(inventory != null, "Inventory not set!")
...
func get_items_under(rect: Rect2i) -> Array[ItemStack]:
  assert(inventory != null, "Inventory not set!")
...
func move_item_to(item: ItemStack, position: Vector2i) -> bool:
  assert(inventory != null, "Inventory not set!")
  var item_size := get_item_size(item)
  var rect := Rect2i(position, item_size)
...
func move_item_to_free_spot(item: ItemStack) -> bool:
  if rect_free(get_item_rect(item), item):
    return true
...
func _merge_to(item: ItemStack, destination: GridConstraint, position: Vector2i) -> bool:
...
func _get_mergable_item_at(item: ItemStack, position: Vector2i) -> ItemStack:
...
func _get_mergable_items_under(item: ItemStack, rect: Rect2i) -> Array[ItemStack]:
  var result: Array[ItemStack]
...
func rect_free(rect: Rect2i, exception: ItemStack = null) -> bool:
  assert(inventory != null, "Inventory not set!")
...
func find_free_place(item: ItemStack, exception: ItemStack = null) -> Dictionary:
...
func _compare_items(item1: ItemStack, item2: ItemStack) -> bool:
...
func sort() -> bool:
  ...
  var item_array: Array[ItemStack]
  for item in inventory.get_items():
...
func _get_free_space_for(item: ItemStack) -> int:
  var item_size = get_item_size(item)

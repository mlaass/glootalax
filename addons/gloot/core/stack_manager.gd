...
static func get_item_stack_size(item: ItemStack) -> _ItemCount:
	assert(item != null, "item is null!")
	var stack_size: int = item.get_property(_KEY_STACK_SIZE, DEFAULT_STACK_SIZE)
	return _ItemCount.new(stack_size)


static func get_item_max_stack_size(item: ItemStack) -> _ItemCount:
	assert(item != null, "item is null!")
	# First check for item-level override, then item_type, then default
	if item.is_property_overridden(_KEY_MAX_STACK_SIZE):
		return _ItemCount.new(item.get_property(_KEY_MAX_STACK_SIZE, DEFAULT_MAX_STACK_SIZE))
	if item.item_type != null:
		return _ItemCount.new(item.item_type.max_stack_size)
	return _ItemCount.new(DEFAULT_MAX_STACK_SIZE)


static func set_item_stack_size(item: ItemStack, stack_size: _ItemCount) -> bool:
	assert(item != null, "item is null!")
...
static func set_item_max_stack_size(item: ItemStack, max_stack_size: _ItemCount) -> void:
	assert(item != null, "item is null!")
	assert(max_stack_size != null, "max_stack_size is null!")
	item.set_property(_KEY_MAX_STACK_SIZE, max_stack_size.count)


static func stacks_compatible(item_1: ItemStack, item_2: ItemStack) -> bool:
	# Two item stacks are compatible for merging if they have the same item_type and neither of the two contain
	...
	# Both must have the same item_type
	if item_1.item_type != item_2.item_type:
		return false
...
static func merge_stacks(item_dst: ItemStack, item_src: ItemStack, split_source: bool = false) -> bool:
	if split_source:
		return _merge_stacks_split_source(item_dst, item_src)
	return _merge_stacks(item_dst, item_src)


static func can_merge_stacks(item_dst: ItemStack, item_src: ItemStack, split_source: bool = false) -> bool:
	if split_source:
		return _can_merge_stacks_split_source(item_dst, item_src)
	return _can_merge_stacks(item_dst, item_src)


static func _merge_stacks(item_dst: ItemStack, item_src: ItemStack) -> bool:
...
static func _can_merge_stacks(item_dst: ItemStack, item_src: ItemStack) -> bool:
...
static func _merge_stacks_split_source(item_dst: ItemStack, item_src: ItemStack) -> bool:
	assert(item_dst != null, "item_dst is null!")
	assert(item_src != null, "item_src is null!")
...
static func _can_merge_stacks_split_source(item_dst: ItemStack, item_src: ItemStack) -> bool:
	assert(item_dst != null, "item_dst is null!")
	assert(item_src != null, "item_src is null!")
	if item_dst == item_src:
		return false
...
static func get_free_stack_space(item: ItemStack) -> _ItemCount:
	assert(item != null, "item is null!")
	return get_item_max_stack_size(item).sub(get_item_stack_size(item))


static func split_stack(item: ItemStack, new_stack_size: _ItemCount) -> ItemStack:
...
static func can_split_stack(item: ItemStack, new_stack_size: _ItemCount) -> bool:
	if get_item_stack_size(item).gt(new_stack_size):
		return true
	return false


static func inv_split_stack(inv: Inventory, item: ItemStack, new_stack_size: _ItemCount) -> ItemStack:
...
static func inv_merge_stack(inv: Inventory, item_dst: ItemStack, item_src: ItemStack, split_source: bool = false) -> bool:
	if split_source:
		return _inv_merge_stack_split_source(inv, item_dst, item_src)
	return _inv_merge_stack(inv, item_dst, item_src)


static func _inv_merge_stack(inv: Inventory, item_dst: ItemStack, item_src: ItemStack) -> bool:
...
static func _inv_merge_stack_split_source(inv: Inventory, item_dst: ItemStack, item_src: ItemStack) -> bool:
	assert(inv.has_item(item_dst), "Inventory must contain item_dst!")
...
static func inv_add_automerge(inv: Inventory, item: ItemStack) -> bool:
...
static func inv_add_autosplit(inv: Inventory, item: ItemStack) -> bool:
...
static func inv_add_autosplitmerge(inv: Inventory, item: ItemStack) -> bool:
	assert(!inv.has_item(item), "Inventory must not contain item!")
...
static func _inv_has_space_for_single_item(inv: Inventory, item: ItemStack) -> bool:
...
static func inv_pack_stack(inv: Inventory, item: ItemStack) -> void:

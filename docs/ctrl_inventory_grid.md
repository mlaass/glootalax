...
* `custom_item_control_scene: PackedScene` - Custom control scene representing an `ItemStack` (must inherit `CtrlInventoryItemBase`). If set to `null`, `CtrlInventoryItem` will be used to represent the item.
...
* `select_inventory_item(item: ItemStack) -> void` - Selects the given inventory item.
* `get_selected_inventory_item() -> ItemStack` - Returns the selected inventory item. If multiple items are selected, it returns the first one.
* `get_selected_inventory_items() -> Array[ItemStack]` - Returns an array of selected inventory items.

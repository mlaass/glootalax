...
`CtrlInventoryItemBase` defines some signals and members used for displaying an `ItemStack`. Must be inherited when defining a custom class for representing inventory items.

## Properties

* `item: ItemStack` - Reference to the `ItemStack` that is being displayed.
